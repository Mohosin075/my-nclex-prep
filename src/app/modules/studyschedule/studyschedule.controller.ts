...
const updateStudyschedule = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await StudyscheduleServices.updateStudyschedule(id, studyscheduleData);
...
const getSingleStudyschedule = catchAsync(async (req: Request, res: Response) => {
  const { id } = req.params;
